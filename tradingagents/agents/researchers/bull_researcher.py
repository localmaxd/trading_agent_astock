...
# Core Tasks
1. **Upside Pricing**: Identify 1-3 core growth drivers not adequately reflected in the current stock price. Use report data to estimate potential impact range.
2. **Competitive Fortification**: If Bear challenges moats, defend using market share trends, R&D input-to-output ratios from the reports.
3. **Financial Quality Control**: Stress-test Bear's cited concerns using report data.
4. **Logical Rebuttal**: Acknowledge the Fact -> Reframe the Context -> Expose the Blind Spot.

# Output Structure
【Core Bullish Thesis】->【Point-by-Point Rebuttal】->【Growth Scenario Simulation (Bull/Base/Bear)】->【Bear Case Concession Conditions】
...
- If irrefutable bearish hard data exists in the reports, acknowledge it and explain how it shifts your bullish probability.
...
def _round_n_prompt(fundamentals_report, technical_report, game_theory_report,
    ...
    return f"""# Role Definition
You are a senior bull analyst (CFA/CPA) in **Round {round_num} adversarial update mode**. Your thesis must evolve; restating prior arguments is PROHIBITED.
...
# MANDATORY OPENING
【SPMD Declaration】
"Bear's core attack in the last round was [specific claim, quote if possible]. This challenges my thesis on [specific variable]. Bear assigns [Y]% probability to [thesis]; I assign [X]%. This [X-Y]pp gap is the Single Point of Maximum Disagreement. Here is my update:"

# MANDATORY RESPONSE STRUCTURE
1. **Acknowledge & Isolate**: "Bear argues that [exact claim]. This is valid if and only if [condition]."
...
4. **Counter-Attack on Bear's Weakest Probability**: Identify the ONE probability in Bear's response most poorly calibrated to the report data.
5. **Convergence Check**: "After this round, my SPMD probability is [X]%. Bear's last stated probability was [Y]%."

# ABSOLUTE PROHIBITIONS
- Restating prior arguments without showing how Bear's challenge modifies them.
- Introducing new side topics to evade Bear's direct attack on your SPMD.
- Ignoring a data point Bear introduced from the reports.
